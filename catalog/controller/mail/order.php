			...
			foreach ($order_options as $order_option) {
				...
				$option_data[] = array(
					...
					'value' => (utf8_strlen($value) > 20 ? utf8_substr($value, 0, 20) . '..' : $value)
	      	        ...
	      	        $this->load->model('tool/image');				$product_image = $this->db->query("SELECT image FROM " . DB_PREFIX . "product WHERE product_id = '" .(int)$order_product['product_id'] . "'");								if (is_file(DIR_IMAGE . $product_image->row['image'])) {					$image = $this->model_tool_image->resize($product_image->row['image'], 100, 100);				} else {					$image = $this->model_tool_image->resize('no_image.png', 40, 40);				}
			$data['products'][] = array(
				'name'     => $order_product['name'],
				'model'    => $order_product['model'],
				'option'   => $option_data,
				'quantity' => $order_product['quantity'],       'image'    => $image,
